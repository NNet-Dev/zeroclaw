...
impl OtelObserver {
    ...
    fn parent_cx_for(&self, turn_id: Option<&str>) -> Context {
        if let Some(tid) = turn_id {
            ...
            if let Some(entry) = active.get(tid) {
                return entry.context.clone();
            }
            // Nested SOP ids retain the root turn id before `:sop:`. Use the
            // root agent span as the OTel parent while the event's own
            // `agent_alias` records the effective step agent.
            if let Some((root_turn_id, _)) = tid.split_once(":sop:")
                && let Some(entry) = active.get(root_turn_id)
            {
                return entry.context.clone();
            }
        }
