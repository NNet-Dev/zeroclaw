...
mod tests {
    ...
    async fn sop_approve_registry_binds_the_calling_agent_alias() {
        ...
        groups.insert(
            "release".to_string(),
            ApprovalGroupConfig {
                members: vec!["agent:alpha".to_string()],
            },
        );
        ...
        let beta_tools = all_tools_with_runtime(
            Arc::new(Config::default()),
            &security,
            &risk,
            "beta",
        ...
        .tools;
        let alpha_tools = all_tools_with_runtime(
            Arc::new(Config::default()),
            &security,
            &risk,
            "alpha",
        ...
        let beta = beta_tools
            .iter()
            .find(|t| t.name() == "sop_approve")
            .expect("beta registry has sop_approve");
        let beta_result = beta
            .execute(serde_json::json!({ "run_id": run_id.clone() }))
            .await
            .unwrap();
        assert!(!beta_result.success);
        assert!(
            beta_result
                .error
                .as_deref()
                .is_some_and(|e| e.contains("Not authorized")),
            "beta must not satisfy agent:alpha membership: {beta_result:?}"
        );
        assert_eq!(
            shared_engine
                .lock()
            ...
            "the unauthorized beta approval must leave the run waiting"
        );

        let alpha = alpha_tools
            .iter()
            .find(|t| t.name() == "sop_approve")
            .expect("alpha registry has sop_approve");
        let alpha_result = alpha
            .execute(serde_json::json!({ "run_id": run_id }))
            .await
            .unwrap();
        assert!(
            alpha_result.success,
            "alpha should satisfy agent:alpha membership: {alpha_result:?}"
        );
    }
