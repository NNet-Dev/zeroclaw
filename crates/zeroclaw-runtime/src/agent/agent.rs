...
mod tests {
    ...
    fn blank_input_agent(model_provider: Box<dyn ModelProvider>) -> Agent {
        ...
        Agent::builder()
            .model_provider(model_provider)
            .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(Vec::new()))
            .memory(mem)
            .observer(observer)
    ...
    async fn direct_agent_turn_stdout_boundary_helper_4721() {
        ...
        let mut agent = Agent::builder()
            .model_provider(model_provider)
            .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(vec![Box::new(MockTool)]))
            .memory(mem)
            .observer(observer)
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
    ...
    mod surface2_tests {
        ...
        fn test_agent_with_provider_and_multimodal(
            ...
            let mut builder = Agent::builder()
                .model_provider(provider)
                .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(tools))
                .memory(mem)
                .observer(observer)
                .workspace_dir(workspace.path().to_path_buf());
    ...
    async fn turn_streamed_cancel_during_tool_execution_emits_agent_end_with_tokens() {
        ...
        let mut agent = Agent::builder()
            ...
            .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(vec![Box::new(SlowTool)]))
            .memory(mem)
            .observer(observer)
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
    ...
    fn turn_streamed_applies_pending_model_switch_for_next_call() {
        ...
        let mut agent = Agent::builder()
            .model_provider(provider)
            .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(vec![Box::new(ModelSwitchTriggerTool {
