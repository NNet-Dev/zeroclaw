...
/// POST a message built from a full [`DiscordOutgoing`] (content + components)
/// AND file attachments via multipart, returning the new message's ID. This is
/// the components-bearing variant of [`send_discord_message_with_files`]: the
/// `payload_json` part carries `content`/`components` together so a message can
/// attach files and interactive components in one send. Components serialize
/// through the same `to_rest_json`/`payload_json` chokepoint, so an action row
/// whose buttons all fail to encode simply omits the key.
pub(crate) async fn send_discord_outgoing_with_files(
    client: &reqwest::Client,
    bot_token: &str,
    recipient: &str,
    outgoing: &DiscordOutgoing,
    ...
    let mut form = Form::new().text("payload_json", outgoing.payload_json());
