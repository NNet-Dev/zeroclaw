//! Minimal GitHub REST client for the channel.
//!
...
use super::types::{
    ...
    GhComment, GhInstallation, GhIssue, GhRepo, GhTokenResponse, GithubChannelError,
    InstallationId, IssueRef, RepoRef,
};
...
impl GithubApi {
    ...
    #[cfg(test)]
    pub(super) fn with_base(base: String) -> Self {
        Self {
            base,
            proxy_url: None,
        }
    ...
    async fn error_from(endpoint: &str, resp: reqwest::Response) -> GithubChannelError {
        let status = resp.status();
        if status.as_u16() == 429 || status.as_u16() == 403 {
            ...
            if remaining == Some("0") || retry_after.is_some() {
                ...
                return GithubChannelError::RateLimited { reset_at };
            }
        }
        let body = resp.text().await.unwrap_or_default();
        GithubChannelError::Api {
    ...
    pub async fn app_slug(&self, jwt: &str) -> Result<String, GithubChannelError> {
    ...
    pub async fn list_installations(
        &self,
        jwt: &str,
    ) -> Result<Vec<GhInstallation>, GithubChannelError> {
    ...
    ) -> Result<CachedToken, GithubChannelError> {
        let url = format!(
    ...
    pub async fn list_installation_repos(
        &self,
        token: &str,
    ) -> Result<Vec<RepoRef>, GithubChannelError> {
    ...
    pub async fn list_issue_comments_since(
        ...
        repo: &RepoRef,
        since: DateTime<Utc>,
    ) -> Result<Vec<GhComment>, GithubChannelError> {
        let url = format!(
    ...
    pub async fn list_issues_since(
        ...
        repo: &RepoRef,
        since: DateTime<Utc>,
    ) -> Result<Vec<GhIssue>, GithubChannelError> {
        let url = format!(
